...
class HX711(object):
    ...
    def calibrate(self, reference_units, scales_value):
        # Calibrates the sensor using a known reference weight.
        # :param reference_units: The reference weight in units of measurement.
        # :param weight: The measured weight corresponding to the reference weight.
        # :param scales_value: The raw value read from the scales.
        if not self.is_ready():
            self._wait()

        # Set the channel to the maximum gain
        original_channel = self._channel
        self._channel = self.CHANNEL_A_128

        num_values = 40  # Broj uzastopnih očitanja za izračun prosječne sirove vrijednosti
        sum_values = 0
        for _ in range(num_values):
            sum_values += self.read(raw=True)

        average_raw_value = sum_values / num_values

        # Calculate the calibration factor
        calibration_factor = reference_units / (average_raw_value - scales_value)

        # Set the channel back to the original value
        self._channel = original_channel
